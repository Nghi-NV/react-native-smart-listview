...
class SmartListView extends PureComponent {
  ...
  _renderRow = (item, sectionID, index) => {
    ...
    return (
      <SmartSwipeRow
        ...
        damping={damping}
        tension={tension}
        left={left.length > 0 ? left.map(l => l.onPress ? { ...l, onPress: () => l.onPress(item), component: () => l.component(item) } : {...l, component: () => l.component(item)}) : left}
        right={right.length > 0 ? right.map(r => r.onPress ? { ...r, onPress: () => r.onPress(item), component: () => r.component(item) } : {...r, component: () => r.component(item)}) : right}
  ...
  _renderHeader = (viewState) => {
    ...
    if (this.props.renderHeader) {
      return this.props.renderHeader(viewState)
    }
  ...
  _renderFooter = (viewState) => {
    ...
    if (this.props.renderFooter) {
      return this.props.renderFooter(viewState)
    }
  ...
  closeRow() {
    if (this._rows[this._openCellId] && this._rows[this._openCellId].row) {
